...
def create_history_repo(root: Path, name: str, extension: str) -> Path:
    ...
    commit_all(repo, "private code-heavy commit message must not leak")
    ...
    commit_all(repo, "private generated control commit must not leak")
    return repo
...
class WorkCommitHistoryTest(unittest.TestCase):
    ...
    def test_dirty_work_commit_history_repo_fails_runtime_gate(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = create_history_repo(Path(tmpdir), "private-ruejai-app", "dart")
